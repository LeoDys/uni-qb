<template>
	<view class="index-list animated fadeInLeft fast">
		<view class="index-list1 u-f-ac u-f-jsb">
			...
			<view class="u-f-ac" v-show="!item.isguanzhu" @tap="guanzhu(index)">
		...
		<view class="index-list4 u-f-ac u-f-jsb">
			<view class="u-f-ac">
				<view class="u-f-ac" :class="{'active':(item.infonum.index==1)}" @tap="caozuo('ding',index)">
					<view class="icon iconfont icon-icon_xiaolian-mian"></view>
					{{item.infonum.dingnum}}
				</view>
				<view class="u-f-ac" :class="{'active':(item.infonum.index==2)}" @tap="caozuo('cai',index)">
			...
			caozuo(type,index) {
				switch (type) {
					case "ding":
						...
						this.$emit('caidingIndex',{"item":this.item,"index":index});
